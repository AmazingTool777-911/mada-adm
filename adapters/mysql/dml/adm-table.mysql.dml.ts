...
export abstract class BaseAdmTableMySQLDML {
  ...
  protected async _getManyByAttributes(
    ...
    if (this.config.hasGeojson) {
      columns.push("ST_AsGeoJSON(t.geojson) as geojson");
    }

    const rowValues = values.map(() =>
      `ROW(${attributes.map(() => "?").join(", ")})`
    ).join(", ");
    ...
    const sql = `
			...
			FROM ${tableName} t
			JOIN v ON ${
      attributes.map((attr) => `t.${attr} = v.${attr}`).join(" AND ")
    }
		`;

    const [res] = (await client.query(sql, values.flat())) as unknown as [
      AdmEntitySnakeCased[],
      unknown,
    ];
  ...
  protected async _updateGeojsonByIdentifiers(
    ...
    const sql = `
			UPDATE ${tableName}
			SET geojson = ST_GeomFromGeoJSON(?),
				updated_at = NOW(3)
			WHERE ${attributes.map((attr) => `${attr} = ?`).join(" AND ")}
		`;

    const [res] =
      (await client.query(sql, [geojson, ...values])) as unknown as [
        { affectedRows: number },
        unknown,
      ];
  ...
  protected async _createMany(
    ...
    return { insertedCount: res.affectedRows };
  }
  ...
  protected async _getManyByParentsIds(
    ...
    const [res] = (await client.query(sql, parentsIds)) as unknown as [
      AdmEntitySnakeCased[],
      unknown,
    ];
  ...
  protected async _updateFieldByIds(
    ...
    const [res] = (await client.query(sql, [value, ...ids])) as unknown as [
      { affectedRows: number },
      unknown,
    ];
    return { affectedRows: res.affectedRows };

...
abstract class GetAdmLevelMySQLQueryBuilder
  ...
  public build(
    ...
    if (search) {
      searchWhereClause =
        `${this.tableName}.${this.targetColumn} LIKE CONCAT(?, '%')`;
    } else {
      searchWhereClause = "";
    }
    ...
    return { sqlTemplate, params, dbType: DbType.MySQL };
  }
}

class GetProvincesMySQLQueryBuilder extends GetAdmLevelMySQLQueryBuilder {
  constructor(config: MadaAdmConfigValues) {
    super(config, AdmLevelCode.PROVINCE);
  }
}

class GetRegionsMySQLQueryBuilder extends GetAdmLevelMySQLQueryBuilder {
  constructor(config: MadaAdmConfigValues) {
    super(config, AdmLevelCode.REGION);
  }
}

class GetDistrictsMySQLQueryBuilder extends GetAdmLevelMySQLQueryBuilder {
  constructor(config: MadaAdmConfigValues) {
    super(config, AdmLevelCode.DISTRICT);
  }
}

class GetCommunesMySQLQueryBuilder extends GetAdmLevelMySQLQueryBuilder {
  constructor(config: MadaAdmConfigValues) {
    super(config, AdmLevelCode.COMMUNE);
  }
}

class GetFokontanyMySQLQueryBuilder extends GetAdmLevelMySQLQueryBuilder {
  constructor(config: MadaAdmConfigValues) {
    super(config, AdmLevelCode.FOKONTANY);
  }
}

export class GetAdmLevelEntitiesOfParentMySQLQueryBuilder
  extends GetAdmLevelEntitiesOfParentQueryBuilder<
    GetAdmLevelEntitiesSqlQueryBody
  > {
  constructor(config: MadaAdmConfigValues) {
    super({
      [AdmLevelCode.PROVINCE]: new GetProvincesMySQLQueryBuilder(config),
      [AdmLevelCode.REGION]: new GetRegionsMySQLQueryBuilder(config),
      [AdmLevelCode.DISTRICT]: new GetDistrictsMySQLQueryBuilder(config),
      [AdmLevelCode.COMMUNE]: new GetCommunesMySQLQueryBuilder(config),
      [AdmLevelCode.FOKONTANY]: new GetFokontanyMySQLQueryBuilder(config),
    });
  }
}

let _instance: GetAdmLevelEntitiesOfParentMySQLQueryBuilder | null = null;

export function injectGetAdmLevelEntitiesOfParentMySQLQueryBuilder(
  config: MadaAdmConfigValues,
) {
  return _instance ??
    (_instance = new GetAdmLevelEntitiesOfParentMySQLQueryBuilder(config));
}

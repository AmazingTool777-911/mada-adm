...
import {
  GetAdmLevelEntitiesOfParentMySQLQueryBuilder,
} from "./get-adm-level-entities-of-parent.mysql.query-builder.ts";
...
function makeBuilder(
  ...
  return new GetAdmLevelEntitiesOfParentMySQLQueryBuilder(config);
}
...
Deno.test(
  ...
  () => {
    ...
    assertEquals(result.dbType, DbType.MySQL);
...
Deno.test("build — regions (ADM1) under province (ADM0), with search and default limit", () => {
  ...
  const sql = normalizeSql(result.sqlTemplate);
  assertEquals(
    sql.includes("regions.region LIKE CONCAT(?, '%')"),
    true,
  );
  assertEquals(
    sql.includes("SELECT id FROM provinces WHERE provinces.id = ? LIMIT ?"),
...
Deno.test(
  ...
  () => {
    ...
    const sql = normalizeSql(result.sqlTemplate);
    assertEquals(
      sql.includes("fokontanys.fokontany LIKE CONCAT(?, '%')"),
      true,
    );
    assertEquals(

 ...
 * using MySQL.
 */
export class DistrictsMySQLDDL extends BaseAdmTableDDL {
  constructor(
  ...
  async create(transactionContext?: DbTransactionContext): Promise<void> {
    ...
    const query = `
      CREATE TABLE IF NOT EXISTS ${this.tableName} (
        ...
        region_id INT NOT NULL,${optionalCols}${admLevelColumn}${geometryColumn}
...
let _instance: DistrictsMySQLDDL | null = null;

export function injectDistrictsMySQLDDL(
  config: MadaAdmConfigValues,
  db: MySQLDbConnection,
): DistrictsMySQLDDL {
  if (!_instance) _instance = new DistrictsMySQLDDL(db, config);
  return _instance;
}

export function resetDistrictsMySQLDDL(): void {
  _instance = null;
}
